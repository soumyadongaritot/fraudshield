// FraudShield v3.1 — Content Script
// Auto-scans every page, sends result to background for notification + CSV
...
function saveToHistory(url, result) {
  chrome.storage.local.get(["scanHistory"], data => {
    ...
    history.unshift({
      ...
      site_type: result.domain_info?.site_type ?? "Unknown",
      domain:    result.domain_info?.domain    ?? "",
      protocol:  result.domain_info?.protocol  ?? "",
...
function injectWarningScreen(result) {
  ...
  overlay.innerHTML = `
    ...
    <div class="fs-warning-box">
      <div class="fs-warning-header">
        <div class="fs-shield-icon">🛡️</div>
        <div><span class="fs-brand">FraudShield</span><span class="fs-version">v3.1 AI</span></div>
      </div>
      <div class="fs-danger-badge">⚠️ DANGEROUS PAGE BLOCKED</div>
      <div class="fs-score-row">
        <div class="fs-score-circle">
          <svg viewBox="0 0 100 100">
            <circle cx="50" cy="50" r="40" fill="none" stroke="#ff000033" stroke-width="8"/>
            <circle cx="50" cy="50" r="40" fill="none" stroke="#ff4444" stroke-width="8"
              stroke-dasharray="${score * 2.51} 251"
              stroke-dashoffset="62.8" stroke-linecap="round"/>
          </svg>
          <div class="fs-score-num">${score}</div>
          <div class="fs-score-label">/100</div>
        </div>
        <div class="fs-score-info">
          <div class="fs-category">${category}</div>
          <div class="fs-risk">${result.risk_level}</div>
          <div class="fs-domain">${domain}</div>
        </div>
      </div>
      ...
      <div class="fs-footer">FraudShield v3.1 — AI Fraud Detection</div>
    </div>`;
  ...
  style.textContent = `
    ...
    .fs-category{color:#ff4444;font-size:18px;font-weight:700}.fs-risk{color:#ff666688;font-size:12px;margin:4px 0}.fs-domain{color:#ffffff66;font-size:13px;word-break:break-all}
...
async function autoScan() {
  try {
    ...
    // 2. Send to background for: notification + CSV + badge
    ...
    // 3. Block dangerous pages
    if (result.safety_score < BLOCK_THRESHOLD) {
      injectWarningScreen(result);
    }
